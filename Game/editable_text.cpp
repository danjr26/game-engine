...
EditableText::EditableText(TextInputContext* i_context, FontFaceRasterSet* i_rasterSet) :
	mInput(i_context),
	mText("text", i_rasterSet),
	mCursorSprite(AABox2d(
		AABox2d::fromExtrema(
			Vector2d(0, 0), 
...
void EditableText::setCursorPosition(uint i_cursorPos) {
	mCursorPosition = geutil::min<uint>(i_cursorPos, (uint)mText.getText().size());
	updateCursorSprite();
...
void EditableText::update(double i_dt) {
	InputEvent _event;
	...
	if (mCursorBlinkStepper.stepTotal(i_dt)) {
