...
Game::Game() :
	...
	mPlayerShipInput(),
	mPlayerShip(nullptr),
	mMainCollisionContext(AABox2d::fromCenter(Vector2d(), Vector2d(1024, 1024)), 10) {
...
void Game::init() {
	...
	Sprite* perlinSprite = new Sprite(AABox2d::fromCenter(Vector2d(), Vector2d::filled(20)), perlinTexture);
	...
	*/
	...
	Sprite* smallBullet = new Sprite(AABox2d::fromCenter(Vector2d(0, 0), Vector2d(1.0, 1.0)), smallBulletTex);
...
void Game::preupdate(double i_dt) {
	mMainCollisionContext.broadcast();
}

void Game::postupdate(double i_dt) {}
