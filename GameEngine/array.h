...
template<class T>
struct Triangle3 : public Triple<Vector3<T>> {
	...
	Triangle3(Vector3<T> const& in_value1, Vector3<T> const& in_value2, Vector3<T> const& in_value3) :
	Triple<Vector3<T>>(value1, value2, value3)
	{}

	Triangle3(const Vector3<T>* in_values) :
	Triple<Vector3<T>>(in_values) 
	{}

	Triangle3(Triple<Vector3<T>> in_source) :
	Triple<Vector3<T>>(in_source) 
	{}
	...
	Vector3<T> Side(uint in_index) {
		return values[(in_index + 1) % 3] - values[in_index];
	}

	T Side_Length(uint in_index) {
		return Side(in_index).Magnitude();
	}

	T Angle(uint in_index) {
		return 180.0 - Side(in_index).Theta(Side((in_index + 2) % 3));
	}
	...
	Vector3<T> Vertex_By_Distance_From(Vector3<T>& in_vertex, SizeClassification in_size) {
		return values[Vertex_Index_By_Distance_From(in_vertex, in_size)];
	}

	T Vertex_Distance_By_Distance_From(Vector3<T>& in_vertex, SizeClassification in_size) {
		return (Vertex_By_Distance_From(in_vertex, in_size) - in_vertex).Magnitude();
	}

	uint Vertex_Index_By_Distance_From(Vector3<T>& in_vertex, SizeClassification in_size) {
		Triple<T> distances;
		for (int i = 0; i < 3; i++) {
	...
	Vector3<T> Side_By_Size(SizeClassification in_size) {
...
public:
	ResponsibleIndexedArray(uint in_maxCount, uint in_indexByteOffset, bool in_ordered) :
	CustomArray<T*>		(in_maxCount, in_ordered), 
	ResponsibleArray<T>	(in_maxCount, in_ordered), 
	IndexedArray<T>		(in_maxCount, in_indexByteOffset, in_ordered)
	{}
