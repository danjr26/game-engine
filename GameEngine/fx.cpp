...
void LightningBolt::Render(Flags in_renderFlags) {
	...
	Texture2* tex = (Texture2*)GEngine::Get().Resource().Get("lightning.tga");
	tex->Activate();
...
void LaserManager::Render(Flags in_callFlags) {
	if (in_callFlags == RenderType::transparent) {
		...
		Texture2* texture = (Texture2*)GEngine::Get().Resource().Get("sparkglow.tga");
		texture->Activate();
		...
		texture = (Texture2*)GEngine::Get().Resource().Get("sparkparticle.tga");
		texture->Activate();
		glDrawArrays(GL_LINES, 0, components.Size() * 2);
