	...
	if (direction.X() == 0.0) {
		if (Is_Between_Inc(point1.X(), minima.X(), maxima.X())) {
			if (sign.Y() != -Sign(minima.Y() - point1.Y()) && sign.Y() != Sign(minima.Y() - point2.Y())) {
				collision.didCollide = true;
				if (returnPoint) {
					collision.collisionPoint = Vector2d(point1.X(), minima.Y());
				}
			}
			else if (sign.Y() != -Sign(maxima.Y() - point1.Y()) && sign.Y() != Sign(maxima.Y() - point2.Y())) {
				collision.didCollide = true;
				if (returnPoint) {
					collision.collisionPoint = Vector2d(point1.X(), maxima.Y());
				}
			}
		}
		return collision;
	...
	if (direction.Y() == 0.0) {
		if (Is_Between_Inc(point1.Y(), minima.Y(), maxima.Y())) {
			if (sign.X() != -Sign(minima.X() - point1.X()) && sign.X() != Sign(minima.X() - point2.X())) {
				collision.didCollide = true;
				if (returnPoint) {
					collision.collisionPoint = Vector2d(minima.X(), point1.Y());
				}
			}
			else if (sign.X() != -Sign(maxima.X() - point1.X()) && sign.X() != Sign(maxima.X() - point2.X())) {
				collision.didCollide = true;
				if (returnPoint) {
					collision.collisionPoint = Vector2d(maxima.X(), point1.Y());
				}
			}
		}
		return collision;
...
Collision2d InPlaceCollisionEvaluator2::Evaluate_Typed(AxisAlignedRectangleCollisionMask& in_mask1, Triangle2CollisionMask& in_mask2) {
	...
	AxisAlignedRectangleCollisionMask aaRectangleMask = AxisAlignedRectangleCollisionMask(rectangle, true);
	Triangle2CollisionMask triangleMask = Triangle2CollisionMask(triangle, true);
	...
	collision = Evaluate_Typed(Point2CollisionMask(rectangle.Get_Center(), true), triangleMask);
...
Collision2d InPlaceCollisionEvaluator2::Evaluate_Typed(CircleCollisionMask& in_mask1, LineSegment2CollisionMask& in_mask2) {
	...
	Vector2d projectionPoint = lineSegment.Get_Projection(circle.Get_Center());

	Point2CollisionMask pointMask1 = Point2CollisionMask(projectionPoint, true);
...
Collision2d InPlaceCollisionEvaluator2::Evaluate_Typed(CircleCollisionMask& in_mask1, Ray2CollisionMask& in_mask2) {
	...
	Vector2d projectionPoint = ray.Get_Projection(circle.Get_Center());
...
Collision2d InPlaceCollisionEvaluator2::Evaluate_Typed(LineSegment2CollisionMask& in_mask1, RectangleCollisionMask& in_mask2) {
	Collision2d collision;
	...
	for (uint i = 0; i < 4; i++) {
		...
		if (Is_Between_Inc(
			...
			testCollision = Evaluate_Typed(Point2CollisionMask(projectionPoint, true), rectangleMask);
				if (testCollision.didCollide) {
					collision = testCollision;
					break;
				}
...
Collision2d InPlaceCollisionEvaluator2::Evaluate_Typed(LineSegment2CollisionMask & in_mask1, Triangle2CollisionMask & in_mask2) {
	Collision2d collision;
...
Collision2d InPlaceCollisionEvaluator2::Evaluate_Typed(Point2CollisionMask& in_mask1, Triangle2CollisionMask& in_mask2) {
	...
	if (Sign(lazyNormals[0].Dot(point - points[0])) <= 0 && 
		Sign(lazyNormals[1].Dot(point - points[1])) <= 0 && 
		Sign(lazyNormals[2].Dot(point - points[2])) <= 0) {
...
Collision2d InPlaceCollisionEvaluator2::Evaluate_Typed(RectangleCollisionMask& in_mask1, Triangle2CollisionMask& in_mask2) {
	Collision2d collision;

	Rectangled rectangle = in_mask1.Get_Transformed_Rectangle();
	Triangle2d triangle = in_mask2.Get_Transformed_Triangle();

	RectangleCollisionMask rectangleMask = RectangleCollisionMask(rectangle, true);
	Triangle2CollisionMask triangleMask = Triangle2CollisionMask(triangle, true);

	Vector2d triangleCorners[3];
	triangle.Get_Points(triangleCorners);

	for (uint i = 0; i < 3; i++) {
		collision = Evaluate_Typed(LineSegment2CollisionMask(LineSegment2d::From_Points(triangleCorners[i], triangleCorners[(i + 1) % 3]), true), rectangleMask);
		if (collision.didCollide) {
			return collision;
		}
	}
	...
	collision = Evaluate_Typed(Point2CollisionMask(triangleCorners[0], true), rectangleMask);
...
Collision2d InPlaceCollisionEvaluator2::Evaluate_Typed(Line2CollisionMask& in_mask1, RectangleCollisionMask& in_mask2) {
	Collision2d collision;
	...
	for (uint i = 0; i < 4; i++) {
		...
		collision = Evaluate_Typed(Point2CollisionMask(projectionPoint, true), rectangleMask);
		if (collision.didCollide) {
			return collision;
		}
	}
...
Collision2d InPlaceCollisionEvaluator2::Evaluate_Typed(Line2CollisionMask& in_mask1, Triangle2CollisionMask& in_mask2) {
	Collision2d collision;
...
Collision2d InPlaceCollisionEvaluator2::Evaluate_Typed(AxisAlignedHalfSpace2CollisionMask& in_mask1, LineSegment2CollisionMask& in_mask2) {
	Collision2d collision;
	...
	Ranged range = Ranged(point1[dimension], point2[dimension]);
...
Collision2d InPlaceCollisionEvaluator2::Evaluate_Typed(AxisAlignedHalfSpace2CollisionMask& in_mask1, Point2CollisionMask& in_mask2) {
	Collision2d collision;

	AxisAlignedHalfSpace2d halfSpace = in_mask1.Get_Transformed_Half_Space();
	Vector2d point = in_mask2.Get_Transformed_Point();

	double pointValue = point[halfSpace.Get_Dimension()];
	double halfSpaceValue = halfSpace.Get_Value();
...
Collision2d InPlaceCollisionEvaluator2::Evaluate_Typed(AxisAlignedHalfSpace2CollisionMask& in_mask1, Ray2CollisionMask& in_mask2) {
	Collision2d collision;

	Ray2d ray = in_mask2.Get_Transformed_Ray();

	Line2CollisionMask lineMask = Line2CollisionMask(Line2d::From_Point_Direction(ray.Get_Point(), ray.Get_Direction()));

	bool thisReturnPoint = returnPoint;
	returnPoint = true;
	Collision2d testCollision = Evaluate_Typed(in_mask1, lineMask);
	returnPoint = thisReturnPoint;
...
Collision2d InPlaceCollisionEvaluator2::Evaluate_Typed(AxisAlignedHalfSpace2CollisionMask& in_mask1, RectangleCollisionMask& in_mask2) {
	Rectangled rectangle = in_mask2.Get_Transformed_Rectangle();
...
Collision2d InPlaceCollisionEvaluator2::Evaluate_Typed(AxisAlignedHalfSpace2CollisionMask& in_mask1, AxisAlignedHalfSpace2CollisionMask& in_mask2) {
	...
	AxisAlignedHalfSpace2d halfSpace1 = in_mask1.Get_Transformed_Half_Space();
	AxisAlignedHalfSpace2d halfSpace2 = in_mask2.Get_Transformed_Half_Space();
...
Collision2d InPlaceCollisionEvaluator2::Evaluate_Typed(AxisAlignedHalfSpace2CollisionMask& in_mask1, AxisAlignedLine2CollisionMask& in_mask2) {
	Collision2d collision;

	AxisAlignedHalfSpace2d halfSpace = in_mask1.Get_Transformed_Half_Space();
	AxisAlignedLine2d line = in_mask2.Get_Transformed_Line();
...
Collision2d InPlaceCollisionEvaluator2::Evaluate_Typed(AxisAlignedHalfSpace2CollisionMask& in_mask1, AxisAlignedRectangleCollisionMask& in_mask2) {
	AxisAlignedRectangled rectangle = in_mask2.Get_Transformed_Rectangle();

	return Evaluate_Typed(
		in_mask1,
		LineSegment2CollisionMask(LineSegment2d::From_Points(rectangle.Get_Minima(), rectangle.Get_Maxima()), true)
	);
}

Collision2d InPlaceCollisionEvaluator2::Evaluate_Typed(AxisAlignedHalfSpace2CollisionMask& in_mask1, CircleCollisionMask& in_mask2) {
	Circled circle = in_mask2.Get_Transformed_Circle();
	...
	return Evaluate_Typed(
		in_mask1,
		LineSegment2CollisionMask(LineSegment2d::From_Points(circle.Get_Extrema(dimension, false), circle.Get_Extrema(dimension, true)), true)
	);
}

Collision2d InPlaceCollisionEvaluator2::Evaluate_Typed(AxisAlignedHalfSpace2CollisionMask& in_mask1, HalfSpace2CollisionMask& in_mask2) {
	Collision2d collision;

	AxisAlignedHalfSpace2d aaHalfSpace = in_mask1.Get_Transformed_Half_Space();
	HalfSpace2d halfSpace = in_mask2.Get_Transformed_Half_Space();

	Line2d line = Line2d::From_Point_Direction(halfSpace.Get_Point(), halfSpace.Get_Direction().Orthogonal());

	AxisAlignedHalfSpace2CollisionMask aaHalfSpaceMask = AxisAlignedHalfSpace2CollisionMask(aaHalfSpace, true);
	Line2CollisionMask lineMask = Line2CollisionMask(line, true);
...
Collision2d InPlaceCollisionEvaluator2::Evaluate_Typed(AxisAlignedHalfSpace2CollisionMask& in_mask1, Line2CollisionMask& in_mask2) {
	Collision2d collision;
