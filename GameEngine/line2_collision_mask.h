...
template<class T>
class Line2CollisionMask : public CollisionMask<T, 2> {
private:
	Line<T, 2> line;

public:
	Line2CollisionMask(const Line<T, 2>& in_line, bool in_ignoreTransform = false);

	Line<T, 2>& Get_Line();
	Line<T, 2> Get_Transformed_Line() const;
	void Apply_Transform() override;
	Line2CollisionMask* Clone() const override;
