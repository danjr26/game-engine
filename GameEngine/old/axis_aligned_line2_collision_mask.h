...
template<class T>
class AALine2CollisionMask : public CollisionMask<T, 2> {
private:
	AxisAlignedLine<T, 2> line;

public:
	AALine2CollisionMask(const AxisAlignedLine<T, 2>& in_line, bool in_ignoreTransform = false);

	AxisAlignedLine<T, 2>& Get_Line();
	AxisAlignedLine<T, 2> Get_Transformed_Line() const;
	void Apply_Transform() override;
	AALine2CollisionMask<T>* Clone() const override;
