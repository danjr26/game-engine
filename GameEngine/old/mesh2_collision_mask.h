...
template<class T>
class MST2CollisionMask : public CollisionMask<T, 2> {
private:
	MeshVertexData mesh;

public:
	MST2CollisionMask(const MeshVertexData& in_mesh, bool in_ignoreTransform = false);

	MeshVertexData& Get_Mesh();
	MeshVertexData Get_Transformed_Mesh() const;
	void Apply_Transform() override;
	MST2CollisionMask<T>* Clone() const override;
