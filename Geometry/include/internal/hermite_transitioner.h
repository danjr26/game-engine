...
struct HermiteTransitioner : Transitioner<LocatedVector<ValT, n>, TimeT> {
	using Transitioner<LocatedVector<ValT, n>, TimeT>::mKeys;
	LocatedVector<ValT, n> evaluate(TimeT i_time) override {
		if (mKeys.empty()) fail();

		i_time = std::max(i_time, 0);
	...
	void fillUniformDurations(TimeT i_val = (TimeT)1) {
	...
	void fillBasicTangents(const Vector<ValT, n>& i_control1, const Vector<ValT, n>& i_control2) {
		if (mKeys.size() == 0) {
			return;
		}
		...
		for (uint i = 0; i < mKeys.size(); i++) {
			const Vector<ValT, n>& v0 = (i == 0) ? i_control1 : mKeys[i - 1].mValue.mPosition;
			const Vector<ValT, n>& v1 = mKeys[i].mValue.mPosition;
			const Vector<ValT, n>& v2 = (i == mKeys.size() - 1) ? i_control2 : mKeys[i + 1].mValue.mPosition;
	...
	void fillCardinalTangents(TimeT i_tension = (TimeT)0) {
		if (mKeys.empty()) return;
		fillCardinalTangents(mKeys.front().mValue.mPosition, mKeys.back().mValue.mPosition, i_tension);
	}

	void fillCardinalTangents(const Vector<ValT, n>& i_control1, const Vector<ValT, n>& i_control2, TimeT i_tension = (TimeT)0) {
		if (mKeys.size() == 0) {
			return;
		}
		else if (mKeys.size() == 1) {
		...
		for (uint i = 0; i < mKeys.size(); i++) {
			const Vector<ValT, n>& v1 = (i == 0) ? i_control1 : mKeys[i - 1].mValue.mPosition;
			const Vector<ValT, n>& v2 = (i == mKeys.size() - 1) ? i_control2 : mKeys[i + 1].mValue.mPosition;
			TimeT t1 = (i == 0) ? mKeys[i].mDuration : mKeys[i - 1].mDuration;
			TimeT t2 = (i == mKeys.size() - 1) ? mKeys[i - 1].mDuration : mKeys[i].mDuration;
			//mKeys[i].mValue.mVector = (v1 - v2) / 2 * ((TimeT)1 - i_tension);
			mKeys[i].mValue.mVector = (v1 - v2) / (t1 + t2) * ((TimeT)1 - i_tension);
			//mKeys[i].mValue.mVector = (v1 * t1 - v2 * t2) / (t1 + t2) / (t1 + t2) * ((TimeT)1 - i_tension);
		}
	}
};
