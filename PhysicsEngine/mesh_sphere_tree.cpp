...
MeshSphereTree<T, n>::MeshSphereTree(const MeshVertexData& i_meshData) :
	mMeshData(i_meshData),
	mRootNode(nullptr) {
	
	if (i_meshData.getMemberDepth(MeshVertexData::MemberID::position) != n ||
		i_meshData.getMemberType(MeshVertexData::MemberID::position) != MeshVertexData::toDataType<T>() ||
		i_meshData.getFaceMode() != MeshVertexData::FaceMode::triangles) {
...
template<class T, uint n>
MeshSphereTree<T, n>::MeshSphereTree(const MeshSphereTree<T, n>& i_other) :
	mMeshData(i_other.mMeshData),
	mRootNode(nullptr) {
...
template<class T, uint n>
void MeshSphereTree<T, n>::operator=(const MeshSphereTree& i_other) {
	destroyTree();

	mMeshData = i_other.mMeshData;

	Node* otherNode = i_other.mRootNode;
...
template<class T, uint n>
void MeshSphereTree<T, n>::applyTransform(const Transform<T, n>& i_transform) {
	mMeshData.applyTransformPoints(i_transform, MeshVertexData::MemberID::position);
	...
	while (!nodeStack.empty()) {
		...
		node->mSphere.applyTransform(i_transform);
		for (uint i = 0; i < 2; i++) {
...
template<class T, uint n>
MeshSphereTree<T, n>::Iterator::Iterator(const MeshSphereTree<T, n>& i_tree) :
	mTree(i_tree),
	mNode(i_tree.mRootNode) 
{}
